...
def start_single_master(processes, port, sync_config, bebop_ip, local_drone_ip, marker_config_file,
    ...
    os.environ['LOCAL_DRONE_IP'] = local_drone_ip
    ...
    processes.append(subprocess.Popen(master_discovery_cmd.split()))
    time.sleep(2)
    ...
    processes.append(subprocess.Popen(bebop_launch_cmd.split()))
    time.sleep(2)
    arlocros_launch_cmd = 'roslaunch rats bebop.launch marker_config_file:="' + \
                          marker_config_file + '" use_threshold:="' + use_threshold + '"'
    processes.append(subprocess.Popen(arlocros_launch_cmd.split()))
